...
| Primary CTA | "Google로 시작하기" (Button outline variant + 컬러 Google G 로고) | 동일 |
| 서브텍스트 | "브라우저 익스텐션은 곧 공개됩니다 · 이력은 웹에서 먼저 입력해두세요." (CTA 아래 warm-500 한 줄) | 동일 |
...
| 필수 범례 | "* 가 붙은 항목은 필수입니다." (섹션 설명 아래 12px warm-500) |
| 이름 레이블 | "\*이름" (accent 색 asterisk prefix, 띄어쓰기 없이) |
| 이메일 레이블 | "이메일" (보조: "구글 계정 이메일이 사용됩니다 · 수정 불가") |
| 전화 레이블 | "전화번호" (선택 필드이지만 `(선택)` 텍스트 미표기 — asterisk 없음이 곧 선택 의미) |
| 소개 레이블 | "한 줄 소개" |
| 소개 placeholder | "예: 5년차 프론트엔드 개발자, 제품 중심 팀 선호" |
| 저장 상태 (clean) | "저장됨 · {시간}" |
| 저장 상태 (dirty) | "· 변경사항 있음" |
| 저장 버튼 (기본 · always-enabled) | "저장" |
| 저장 버튼 (saving) | "저장 중…" (disabled) |
| 저장 버튼 (saved & 변경 없음) | "저장됨" (enabled — 재클릭은 no-op) |
| 이름 검증 오류 | "이름을 입력해주세요" |
| 전화 검증 오류 | "전화번호 형식이 올바르지 않습니다" |
| 소개 검증 오류 | "한 줄 소개는 200자 이하여야 합니다" |
| 서버 오류 블록 | 에러 타입별 분기 (세션 만료 / 권한 / 네트워크 / 기타) — `mapSaveError`가 동적으로 결정. + [다시 시도] 버튼 |
...
5. ~~**ProfileForm을 `Button`만이 아닌 `Input`/`Textarea` 공용 컴포넌트로 승격할지**~~ — **해결됨**: `FormField` 프리미티브를 `packages/ui`에 승격. react-hook-form + FormField 조합으로 입력 요소는 네이티브 유지, 라벨/asterisk/에러/ARIA는 프리미티브가 담당.
